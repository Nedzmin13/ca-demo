...
function AdminHowToCategoriesPage() {
    ...
    const loadCategories = async () => {
        setLoading(true);
        try {
            const response = await fetchHowToCategoriesForAdmin();
    ...
    const onSubmit = async (data) => {
        try {
            if (editingCategory) {
                await updateHowToCategory(editingCategory.id, data);
            } else {
                await createHowToCategory(data);
            }
            reset();
    ...
    const handleDelete = async (categoryId) => {
        if (window.confirm("Sei sicuro di voler eliminare questa categoria? TUTTI gli articoli al suo interno verranno cancellati.")) {
            try {
                await deleteHowToCategory(categoryId);
                loadCategories();
            } catch (error) {
    ...
    return (
        <>
            <Helmet><title>Gestione Categorie "Come Fare" - Admin</title></Helmet>
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-1">
                    <div className="bg-white p-6 rounded-lg shadow-md sticky top-8">
                        ...
                        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                            ...
                            <div><label>Nome Icona (da Lucide)</label><input {...register('iconName')} className="w-full border p-2 rounded mt-1" placeholder="Es. HardDrive, Smartphone" /></div>
                ...
                <div className="lg:col-span-2">
                    <div className="bg-white p-6 rounded-lg shadow-md">
                        <h2 className="text-2xl font-bold mb-4">Categorie "Come Fare" ({categories.length})</h2>
                        <div className="space-y-3">
                            {loading ? <p>Caricamento...</p> : categories.map(category => (
                                <div key={category.id} className="border p-4 rounded-lg flex justify-between items-center group">
                                    <Link to={`/admin/howto/category/${category.id}`} state={{ categoryName: category.name }} className="flex-grow">
                                        <h3 className="font-bold text-lg group-hover:text-sky-600">{category.name}</h3>
                                        <p className="text-sm text-gray-500">{category._count.articles} articoli</p>
                                    </Link>
...
export default AdminHowToCategoriesPage;